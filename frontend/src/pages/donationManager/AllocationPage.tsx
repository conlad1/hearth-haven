...
const AllocationPage = forwardRef<AllocationPageHandle, AllocationPageProps>(
  function AllocationPage(
    ...
    async function loadAllocations() {
      ...
      try {
        ...
        setError(
          e instanceof Error ? e.message : 'Failed to load allocations.'
        );
      } finally {
        setLoading(false);
      }
    ...
    async function handleDelete(id: number) {
      try {
        const res = await apiFetch(`${API}/Allocation/${id}`, {
          method: 'DELETE',
        });
        if (!res.ok) {
                  ...
                  <tbody>
                    {allocations.map((a) => (
                      ...
                      >
                        ...
                        <td>
                          USD{' '}
                          {Number(a.amountAllocated).toLocaleString('en-US', {
                  ...
                  <div className="space-y-4 rounded-xl bg-gray-50 p-4 dark:bg-white/5">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      {modalMode === 'view' ? (
                        <span className="text-sm font-medium text-gray-900 dark:text-white">
                          {donations.find(
                            (d) => d.donationId === form.donationId
                          )?.label ?? '\u2014'}
                        </span>
                      ) : (
                        <select
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              donationId: Number(e.target.value),
                            })
                          }
                          disabled={!!editing}
                        >
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      {modalMode === 'view' ? (
                        <span className="text-sm font-medium text-gray-900 dark:text-white">
                          {safehouses.find(
                            (s) => s.safehouseId === form.safeHouseId
                          )?.name ?? '\u2014'}
                        </span>
                      ) : (
                        <select
                      ...
                      {modalMode === 'view' ? (
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              amountAllocated: Number(e.target.value),
                            })
                          }
                        />
                      )}
                    ...
                    <div className="mt-6 flex items-center justify-end gap-3">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          handleDelete(editing!.allocationId);
                        }}
                        disabled={saving}
                      >
